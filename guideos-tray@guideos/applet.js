...
UpdatesNotifier.prototype = {
    ...
    _buildMenu: function (count) {
        ...
        let iPrimo = new PopupMenu.PopupIconMenuItem(_("Open Primo"), "applications-system-symbolic", St.IconType.SYMBOLIC);
        iPrimo.connect('activate', () => {
            Util.spawn_async(['primo-di-tutto']);
        });

        let iReportError = new PopupMenu.PopupIconMenuItem(_("Report error"), "dialog-error-symbolic", St.IconType.SYMBOLIC);
        iReportError.connect('activate', () => {
            Util.spawn(['guideos-ticket-tool']);
        });

        let iOpenUpdater = new PopupMenu.PopupIconMenuItem(_("Open Updater"), "software-update-available-symbolic", St.IconType.SYMBOLIC);
